...
public class PostgresDBProperties {
	...
	public PostgresDBProperties() {
		loadProperties("cfg/database/postgres_db.properties");
	}
	
	public PostgresDBProperties(String propertyFilePath) {
	...
	public void loadProperties(String configFile) {
		...
		try {
			...
			try {
				...
				InputStream input = classLoader.getResourceAsStream("postgres_db.properties");
				prop = new Properties();
				prop.load(input);
			}
